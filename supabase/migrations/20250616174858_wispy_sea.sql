/*
  ...
  1. New Columns
    ...
    - `usage_limits` (jsonb) - Usage limits per subscription tier
    - `usage_counts` (jsonb) - Current usage tracking

  2. Constraints
    - Check constraints for valid subscription tiers and statuses

  3. Triggers
    - Auto-update is_pro based on subscription tier and status

  4. Security
    - Updated RLS policies for subscription data
    - Users cannot modify their own subscription details directly

  5. Functions
    - Monthly usage reset function for billing cycles
*/
...
BEGIN
  ...
  -- Create new policies (without OLD/NEW references which are not available in RLS)
...
-- Create a function that only allows service role to update subscription details
CREATE OR REPLACE FUNCTION update_user_subscription(
  user_id_param uuid,
  subscription_tier_param text DEFAULT NULL,
  subscription_status_param text DEFAULT NULL,
  subscription_id_param text DEFAULT NULL,
  trial_ends_at_param timestamptz DEFAULT NULL
)
RETURNS void
SECURITY DEFINER
...
AS $$
BEGIN
  -- This function can only be called by the service role or admins
  IF NOT (
    auth.role() = 'service_role' OR 
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND is_admin = true)
  ) THEN
    RAISE EXCEPTION 'Insufficient permissions to update subscription details';
  END IF;

  UPDATE profiles 
  SET 
    subscription_tier = COALESCE(subscription_tier_param, subscription_tier),
    ...
    trial_ends_at = COALESCE(trial_ends_at_param, trial_ends_at),
    updated_at = now()
